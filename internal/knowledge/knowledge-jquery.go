...
func getJQueryBits() []KnowledgeBit {
	return []KnowledgeBit{
		...
		},
		{
			...
			Description: "Use $.extend() to merge user options with defaults. User options override, missing = use default.",
			HTMLCode:    "<div class=\"modal\">\n  <div class=\"title\">Default</div>\n  <button class=\"close\">✕</button>\n</div>",
			JQueryCode:  "// Plugin with configurable options\n$.fn.modal = function(options) {\n    const settings = $.extend({\n        width: 400, title: 'Modal'\n    }, options);\n    \n    return this.each(function() {\n        $(this).css({ width: settings.width })\n            .find('.title').text(settings.title);\n    });\n};\n\n// Usage\n$('.modal').modal({ width: 600 });              // Uses default 'Modal'\n$('.modal').modal({ title: 'Settings' });        // Uses default 400px\n$('.modal').modal({ width: 600, title: 'Settings' });",
			ModernCode:  "// Modern with spread operator\nclass Modal {\n    constructor(element, options = {}) {\n        this.settings = { width: 400, title: 'Modal', ...options };\n        this.element = element;\n        this.init();\n    }\n    \n    init() {\n        this.element.style.width = `${this.settings.width}px`;\n        this.element.querySelector('.title').textContent = this.settings.title;\n    }\n}\n\nnew Modal(document.querySelector('.modal'), { width: 600 });",
			Category:    "Plugin Development",
		},
		{
		...
		},
		{
			...
			Description: "Chain Deferred with AJAX for complex workflows. Use .then() to transform data between steps.",
			JQueryCode:  "// Chain multiple async operations\nfunction loadUser(userId) {\n    return $.ajax('/api/user/' + userId)\n        .then(user => {\n            return $.ajax('/api/posts', { data: { userId: user.id } })\n                .then(posts => ({ user, posts }));\n        });\n}\n\n// Usage\nloadUser(123)\n    .done(data => render(data.user, data.posts))\n    .fail(showError);\n\n// Simple validation\nfunction validateEmail(email) {\n    const dfd = $.Deferred();\n    email.includes('@') && email.includes('.') \n        ? dfd.resolve('Valid') : dfd.reject('Invalid');\n    return dfd.promise();\n}\n\nvalidateEmail('test@example.com')\n    .then(() => $.ajax('/api/validate'))\n    .done(() => console.log('Validated'))\n    .fail(err => console.error('Error:', err));",
			ModernCode:  "// Modern native Promises\nasync function loadUser(userId) {\n    const user = await fetch('/api/user/' + userId).then(r => r.json());\n    const posts = await fetch('/api/posts?userId=' + user.id).then(r => r.json());\n    return { user, posts };\n}\n\n// Usage\nloadUser(123)\n    .then(data => render(data.user, data.posts))\n    .catch(showError);\n\n// Or try/catch\ntry {\n    const data = await loadUser(123);\n    render(data.user, data.posts);\n} catch (err) { showError(err); }",
			Category:    "Deferred/Promises",
		},
		{
		...
		{
			Title:       "$.map() / $.grep()",
			Description: "$.map() transforms each element and returns new array. $.grep() filters array by condition. Both work on arrays and jQuery collections. Now replaced by native .map() and .filter().",
			JQueryCode:  "// $.map(): Transform array (MUST return value or null)\nconst prices = [10, 20, 30];\nconst withTax = $.map(prices, n => n * 1.1);  // [11, 22, 33]\n\n// $.grep(): Filter array by condition (returns matching items)\nconst expensive = $.grep(prices, n => n > 15);  // [20, 30] (only > 15)\nconst salesItems = $.grep(prices, n => n < 20); // [10] (only < 20)\n\n// Transform jQuery collection to array of values\n// $.map iterates over elements and extracts values\nconst values = $.map($('.price'), function(input) {\n    // Transform element to its value\n    return parseFloat($(input).val());\n});  // [10, 25, 5]\n\n// Filter with index\nconst middle = $.grep(prices, function(n, i) {\n    return i > 0 && n > 15;  // skip first, only > 15\n});",
			ModernCode:  "// Native methods (preferred in modern code)\n// Map: transform each element\nconst withTax = prices.map(n => n * 1.1);\n\n// Filter: keep elements matching condition\nconst expensive = prices.filter(n => n > 15);\n\n// Extract values from elements\nconst values = [...document.querySelectorAll('.price')]\n    .map(input => parseFloat(input.value));\n\n// With index parameter (optional)\nconst middle = prices.map((n, i) => i > 0 ? n : null)\n    .filter(n => n !== null && n > 15);",
			Category:    "Utilities",
		},
		{
		...
		{
			Title:       "Module Pattern - IIFE with Init",
			Description: "Use IIFE pattern with init() function. Check if required elements exist before initializing. Safe on any page.",
			JQueryCode:  "// Module pattern: Only init on correct page\nvar UserManagement = (function($) {\n    'use strict';\n    var $userTable, $deleteBtn;\n    \n    function init() {\n        // Exit if wrong page\n        if ($('#user-management-page').length === 0) return;\n        \n        // Cache and bind if elements exist\n        $userTable = $('#user-table');\n        $deleteBtn = $('.js-delete-user');\n        $deleteBtn.on('click.userMgmt', handleDelete);\n    }\n    \n    function handleDelete(e) { /* ... */ }\n    \n    return { init: init };\n})(jQuery);\n\n// main.js - safe to call on ANY page\n$(document).ready(function() {\n    UserManagement.init();    // Exits early if wrong page\n    Dashboard.init();\n    Reports.init();\n});",
			ModernCode:  "// Modern ES6 class\nclass UserManagement {\n    static init() {\n        const page = document.getElementById('user-management-page');\n        if (!page) return; // Exit early if wrong page\n        \n        this.userTable = document.getElementById('user-table');\n        this.deleteBtn = document.querySelectorAll('.js-delete-user');\n        \n        this.deleteBtn.forEach(btn => \n            btn.addEventListener('click', this.handleDelete.bind(this))\n        );\n    }\n    \n    static handleDelete(e) { /* ... */ }\n}\n\n// Safe to call on any page\nUserManagement.init();",
			Category:    "Best Practices",
		},
		{
		...
		},
		{
			...
			Description: "Build HTML/elements in memory, then insert once. DOM operations are SLOW - each append forces browser to recalculate layout. Batch for performance.",
			JQueryCode:  "// BAD: 100 separate DOM operations!\n// Each .append() forces browser to:\n// 1. Parse HTML string\n// 2. Create element\n// 3. Insert into DOM\n// 4. Recalculate layout\n// RUNS 100 TIMES = very slow!\nfor (let i = 0; i < 100; i++) {\n    $('#list').append('<li>' + i + '</li>');  // Slow: DOM touched 100x\n}\n\n// GOOD: Build array in memory, insert once\n// Build array (in memory, FAST)\nconst items = [];\nfor (let i = 0; i < 100; i++) {\n    items.push('<li>' + i + '</li>');\n}\n// Single DOM operation: join and insert\n$('#list').html(items.join(''));  // ✓ ONE DOM touch = fast!\n\n// EVEN BETTER: Use array methods (cleaner code)\nconst html = Array.from({length: 100}, (_, i) => `<li>${i}</li>`).join('');\n$('#list').html(html);\n\n// Real example: Populate dropdown with 500 options\nconst options = data.map(item => `<option value=\"${item.id}\">${item.name}</option>`).join('');\n$('#dropdown').html(options);  // Fast: 1 DOM write",
			ModernCode:  "// Modern JS: Same performance principle\n// Build array, single write to DOM\nconst items = Array.from({length: 100}, (_, i) => `<li>${i}</li>`);\nlist.innerHTML = items.join('');  // Fast: 1 DOM write\n\n// Better for many elements: use DocumentFragment\n// Avoids innerHTML parsing, creates actual elements\nconst frag = document.createDocumentFragment();\nfor (let i = 0; i < 100; i++) {\n    const li = document.createElement('li');\n    li.textContent = i;\n    frag.appendChild(li);\n}\nlist.appendChild(frag);  // ✓ ONE DOM touch, elements created directly",
			Category:    "Best Practices",
		},
		{
			Title:       "this vs $(this)",
