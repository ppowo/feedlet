...
func getJQueryBits() []KnowledgeBit {
	return []KnowledgeBit{
		...
		},
		{
			...
			Description: "Fetch HTML and insert into element. Can load page fragment with selector. Unique to jQuery (no direct modern equivalent).",
			HTMLCode:    "<div id=\"content\"><!-- Content will be loaded here --></div>\n<aside id=\"sidebar\"><!-- Sidebar widget here --></aside>",
			JQueryCode:  "// Load entire page into element\n$('#content').load('/pages/home.html');\n\n// Load specific fragment (powerful!)\n$('#content').load('/pages/home.html #main-content');\n\n// With callback:\n$('#sidebar').load('/widgets/user-info.html', function(response, status) {\n    if (status === 'success') {\n        console.log('Loaded successfully');\n    }\n});",
			ModernCode:  "fetch('/pages/home.html')\n    .then(res => res.text())\n    .then(html => mainContent.innerHTML = html);\n// Fragment requires parsing:\nconst doc = new DOMParser().parseFromString(html, 'text/html');\nmainContent.innerHTML = doc.querySelector('#content').innerHTML;",
			Category:    "AJAX",
		},
		{
		...
		{
			Title:       "$.fn.extend - Plugin with Options",
			Description: "Create configurable plugins with default settings using $.extend(). Use .each() for multiple elements.",
			HTMLCode:    "<div class=\"modal-window\">\n  <div class=\"title\">Default Title</div>\n  <div class=\"content\">Modal content here</div>\n  <button class=\"close\">Close</button>\n</div>",
			JQueryCode:  "// Plugin with options and callbacks\n$.fn.modal = function(options) {\n    const settings = $.extend({\n        width: 400,\n        height: 300,\n        title: 'Modal',\n        onClose: function() {}\n    }, options);\n    \n    return this.each(function() {\n        const $modal = $(this);\n        \n        // Apply settings to each modal\n        $modal.css({ \n            width: settings.width, \n            height: settings.height \n        });\n        \n        $modal.find('.title').text(settings.title);\n        $modal.find('.close').on('click', settings.onClose);\n    });\n};\n\n// Usage with options:\n$('.modal-window').modal({ \n    width: 600, \n    title: 'Settings',\n    onClose: function() { alert('Closed!'); }\n});",
			ModernCode:  "class Modal {\n    constructor(element, options) { \n        this.element = element;\n        this.options = Object.assign({\n            width: 400,\n            height: 300,\n            title: 'Modal',\n            onClose: function() {}\n        }, options);\n        \n        this.init();\n    }\n    \n    init() {\n        this.element.style.width = `${this.options.width}px`;\n        this.element.style.height = `${this.options.height}px`;\n        const title = this.element.querySelector('.title');\n        if (title) title.textContent = this.options.title;\n        const close = this.element.querySelector('.close');\n        if (close) close.addEventListener('click', this.options.onClose);\n    }\n    \n    show() { this.element.style.width = `${this.options.width}px`; }\n}\n\n// Usage:\nconst modals = document.querySelectorAll('.modal-window');\nconst modalInstance = new Modal(modals[0], { \n    width: 600, \n    title: 'Settings',\n    onClose: () => alert('Closed!')\n});",
			Category:    "Plugin Development",
		},
		{
			Title:       "Plugin Best Practices",
			Description: "Maintain chainability, handle elements individually, provide defaults, support both getter/setter patterns.",
			HTMLCode:    "<div class=\"text\">Sample text</div>\n<div class=\"colorful-box\">Box 1</div>\n<div class=\"colorful-box\">Box 2</div>",
			JQueryCode:  "// GOOD Plugin: Chainable + Individual handling\n$.fn.myPlugin = function(options) {\n    const settings = $.extend({ color: 'red', size: 14 }, options);\n    \n    return this.each(function() {\n        const $this = $(this);\n        $this.css({\n            color: settings.color,\n            fontSize: settings.size + 'px'\n        });\n    });\n};\n\n// BAD: Doesn't return this, breaks chaining\n$.fn.badPlugin = function() {\n    this.css('color', 'blue');\n    // Missing 'return this' - breaks chain!\n};\n\n// Getter/Setter pattern (like .css(), .attr())\n$.fn.dataValue = function(value) {\n    if (value === undefined) {\n        // GETTER: return first element's data\n        return this.data('value');\n    }\n    // SETTER: set all elements' data\n    return this.data('value', value);\n};",
			ModernCode:  "class Plugin {\n    constructor(elements, options) {\n        this.elements = Array.from(elements);\n        this.options = { color: 'red', size: 14, ...options };\n        return this; // enable chaining\n    }\n    \n    css(styles) {\n        this.elements.forEach(el => Object.assign(el.style, styles));\n        return this; // enable chaining\n    }\n    \n    dataValue(value) {\n        if (value === undefined) {\n            // GETTER: return first element's data\n            return this.elements[0]?.dataset.value;\n        }\n        // SETTER: set all elements' data\n        this.elements.forEach(el => el.dataset.value = value);\n        return this;\n    }\n}\n\n// Usage similar to jQuery:\nconst plugin = new Plugin(document.querySelectorAll('.colorful-box'), { color: 'blue', size: 16 });\nplugin.css({ fontWeight: 'bold' }).dataValue('active');",
			Category:    "Plugin Development",
		},
		{
			Title:       "Plugin Instance Management",
			Description: "Use .data() to store plugin instance per element. Prevents multiple initialization conflicts.",
			HTMLCode:    "<div class=\"tab-container\">\n  <button class=\"tab active\">Tab 1</button>\n  <button class=\"tab\">Tab 2</button>\n  <button class=\"tab\">Tab 3</button>\n  <div class=\"panel\">Content 1</div>\n  <div class=\"panel\" style=\"display:none;\">Content 2</div>\n  <div class=\"panel\" style=\"display:none;\">Content 3</div>\n</div>",
			JQueryCode:  "// Plugin that avoids double-initialization\n$.fn.tabs = function(options) {\n    return this.each(function() {\n        const $this = $(this);\n        \n        // Check if already initialized\n        if ($this.data('tabs-instance')) {\n            return; // Already has tabs\n        }\n        \n        // Store instance data\n        const instance = {\n            $tabs: $this.find('.tab'),\n            $panels: $this.find('.panel'),\n            showTab: function(index) {\n                this.$panels.hide().eq(index).show();\n                this.$tabs.removeClass('active').eq(index).addClass('active');\n            }\n        };\n        \n        $this.data('tabs-instance', instance);\n        \n        // Initialize tabs\n        $this.find('.tab').on('click', function() {\n            const index = $(this).index();\n            instance.showTab(index);\n        });\n    });\n};",
			ModernCode:  "// WeakMap for private data - prevents memory leaks\nconst instances = new WeakMap();\n\nclass TabManager {\n    constructor(element) {\n        this.element = element;\n        this.tabs = element.querySelectorAll('.tab');\n        this.panels = element.querySelectorAll('.panel');\n        \n        this.init();\n    }\n    \n    init() {\n        this.tabs.forEach((tab, index) => {\n            tab.addEventListener('click', () => this.showTab(index));\n        });\n    }\n    \n    showTab(index) {\n        this.panels.forEach(panel => panel.style.display = 'none');\n        this.tabs.forEach(tab => tab.classList.remove('active'));\n        \n        this.panels[index].style.display = 'block';\n        this.tabs[index].classList.add('active');\n    }\n    \n    static create(element) {\n        // Check if already initialized\n        if (instances.has(element)) {\n            return instances.get(element);\n        }\n        \n        const instance = new TabManager(element);\n        instances.set(element, instance);\n        return instance;\n    }\n}\n\n// Usage:\nconst tabContainers = document.querySelectorAll('.tab-container');\ntabContainers.forEach(container => TabManager.create(container));",
			Category:    "Plugin Development",
		},
		{
		...
		},
		{
			...
			JQueryCode:  "// Real use case: File upload with Deferred\nfunction uploadFile(file) {\n    const dfd = $.Deferred();\n    const formData = new FormData();\n    formData.append('file', file);\n    \n    $.ajax({\n        url: '/upload',\n        data: formData,\n        processData: false,\n        success: dfd.resolve,  // Delegate success to Deferred\n        error: dfd.reject     // Delegate error to Deferred\n    });\n    \n    return dfd.promise();\n}\n\n// Usage:\nuploadFile(fileInput.files[0])\n    .done(response => showSuccess('File uploaded!'))\n    .fail(xhr => showError('Upload failed'))\n    .always(() => hideProgress());\n    \n// Another use case: API validation\nfunction validateEmail(email) {\n    const dfd = $.Deferred();\n    \n    if (!email.includes('@')) {\n        dfd.reject('Invalid email format');\n        return dfd.promise();\n    }\n    \n    $.get('/api/validate-email', { email })\n        .done(dfd.resolve)\n        .fail(dfd.reject);\n    \n    return dfd.promise();\n}",
		...
		},
		{
			...
			JQueryCode:  "Chain operations with Deferred and AJAX\nfunction loadUserProfile(userId) {\n    return $.ajax('/api/user/' + userId)\n        .then(profile => {\n            // Load more data based on profile\n            return $.ajax('/api/user/posts', {\n                data: { userId: profile.id }\n            });\n        })\n        .then(posts => {\n            // Process posts, return full data\n            return {\n                profile: profile,\n                posts: posts\n            };\n        })\n        .catch(err => {\n            console.error('Failed to load user data:', err);\n            return $.Deferred().reject(err).promise();\n        });\n}\n\n// Usage\nloadUserProfile(123)\n    .done(data => renderUserDashboard(data.profile, data.posts))\n    .fail(showErrorMessage);\n    \n// Manual promise with validation\nfunction validateEmail(email) {\n    const dfd = $.Deferred();\n    \n    if (!email.includes('@')) {\n        dfd.reject('Invalid email');\n    } else {\n        $.get('/api/validate-email', { email })\n            .done(dfd.resolve)\n            .fail(dfd.reject);\n    }\n    \n    return dfd.promise();\n}",
			ModernCode:  "Native Promises\nfunction asyncOp() {\n    return new Promise((resolve, reject) => {\n        setTimeout(() => {\n            Math.random() > 0.3 ? resolve('Success!') : reject('Failed');\n        }, 1000);\n    });\n}",
			Category:    "Deferred/Promises",
		},
		{
		...
		{
			Title:       "Migration to Modern JavaScript",
			Description: "Gradual migration strategy. Replace jQuery DOM manipulation first, keep AJAX until later. Use feature detection.",
			JQueryCode:  "// Migration Strategy: Keep jQuery for complex parts, replace simple DOM ops\n\n// Phase 1: Replace simple selectors and DOM ops\n// OLD:\n$('.item').addClass('active').css('color', 'red');\n// NEW:\ndocument.querySelectorAll('.item').forEach(el => {\n    el.classList.add('active');\n    el.style.color = 'red';\n});\n\n// Phase 2: Keep complex utilities\n// Keep jQuery for animations, event delegation, utilities\n$(document).on('click', '.dynamic-content button', handler);\n\n// Phase 3: Replace AJAX last\n// Old jQuery (keep for now):\n$.ajax({ url: '/api/data', success: displayData });\n// New modern (replace later):\nfetch('/api/data').then(r => r.json()).then(displayData);\n\n// Feature detection for mixed codebase:\nfunction $(selector) {\n    return window.jQuery ? window.jQuery(selector) : \n           document.querySelectorAll(selector);\n}\n\n// Use polyfills for browser compatibility\nif (!window.Promise) {\n    // Load promise polyfill\n}\nif (!window.fetch) {\n    // Keep using jQuery AJAX polyfill\n}",
			Category:    "Version Migration",
		},
		...
		},
		{
			...
			HTMLCode:    "<button id=\"btn\">Click me</button>",
			JQueryCode:  "// WRONG: Creates global variable (missing 'var'/'const')\n$('#btn').on('click', function() {\n    counter = 0;  // NO var/let/const = GLOBAL!\n    counter++;    // Pollutes global scope\n});\nconsole.log(window.counter);  // 1 (OOPS!)\n\n// CORRECT: Use var/let/const\n$('#btn').on('click', function() {\n    let counter = 0;  // Local to this function\n    counter++;\n});\n\n// FIX: Use 'use strict' to catch these errors\n(function($) {\n    'use strict';  // Throws error if you forget var/let/const\n    \n    $('#btn').on('click', function() {\n        counter = 0;  // ERROR: counter is not defined\n    });\n})(jQuery);\n\n// CORRECT with strict mode:\n(function($) {\n    'use strict';\n    \n    $('#btn').on('click', function() {\n        const counter = 0;  // ✓ Local variable\n        counter++;\n    });\n})(jQuery);",
